...
A practical benchmark suite for finding the best MTU + air-gap settings on AS32 RF433 modules using real file-style payload transfer.
...
For each `(MTU, gap_ms)` point, the benchmark sends the same payload (real input file or generated bytes) and records:
- RF throughput (`rf_throughput`, B/s)
- Chunk completion (`chunks_received/chunks_expected`)
- Loss (%)
- CRC failure rate (%)
- Timeout count
...
```bash
python benchmark_tx_fresh.py --file-size 65536 --mtu-list 128,160,256,512 --gap-list 0,1,2,3,5 --repeats 3
```

Use a real file instead of generated bytes:

```bash
python benchmark_tx_fresh.py --input-file sample.bin --mtu-list 128,160,256 --gap-list 0,1,2 --repeats 3
```

CLI options (TX):
- `--file-size`: payload size in bytes (generated payload, or truncate `--input-file`)
- `--input-file`: optional real file path to benchmark with
- `--mtu-list`: comma-separated MTU list (example `128,160,256`)
- `--gap-list`: comma-separated inter-frame gap list in milliseconds
- `--repeats`: repeats per `(MTU, gap_ms)` point
